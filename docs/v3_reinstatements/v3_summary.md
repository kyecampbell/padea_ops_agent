...
A student who departs and returns more than once is a known limitation in V3: the schema's single `current_period_start_date` field is overwritten on each return, so only the most recent return date is directly queryable from the enrolments table. Earlier period start dates are preserved in the agent_steps audit trail but are not structured data. This is an accepted gap at Padea's current scale — the case is rare — and is documented in `edge_cases.md` with a V4 trigger to add a dedicated period history table if it becomes routine.
...
## V4: post-review optimisations

After V3 was finalised and deployed a review pass identified a meaningful cluster of improvements worth capturing before any tool code is written. None of the changes alter the system's behaviour or contradict V3's ten design decisions; they are all structural refinements that would otherwise show up as silent bugs or avoidable query complexity later. The five changes — auto-maintained `updated_at` timestamps, three text-plus-CHECK columns promoted to proper Postgres enum types, a UNIQUE constraint on `outbound_emails.gmail_message_id`, a denormalised `caterer_id` on the `feedback` table for rolling-mean query performance, and a snapshotted `gst_rate_percent` on the `orders` table — are documented in full in `docs/v4_optimised/optimisations.md`. The V4 schema lives in `docs/v4_optimised/Schema/`. V3 is preserved unchanged as the historical record.

---

## Schema note: dietary tags as shared vocabulary

An important design decision in V3 is that dietary properties on menu items are not stored as boolean columns — there is no `is_halal`, `is_vegetarian`, `contains_nuts`, or any equivalent flag on the `menu_items` table. Instead, menu items carry dietary properties through the same `dietary_tags` vocabulary used for student requirements, via a `menu_item_dietary_tags` junction table. This mirrors the `enrolment_dietary_tags` pattern already used on the student side.
