...
**Each operational week.** The system orders one meal per kid from their approved set, picking least-recently-served so the same meal doesn't recur weeks running by chance. Dietary kids get a dietary-compatible meal auto-chosen. If a kid has a meal request filed via the tutor app (see below) for the upcoming week, that request overrides the rotation for that kid only — they get the meal they asked for. Order goes Thursday for the following week. V2's underlying order workflow (exclusions, absences, MOQ, escalations) continues unchanged.
...
5. **Complexity score:** 4/5. Multiple new tables, multi-week rotation chain with several drafted-and-approved email types, two new app surfaces (tutor + manager input boxes), tutor-app per-week request picker, tutor-led preference-reset workflow, per-caterer rolling math, configurable thresholds. Drags in tutors as modelled entities, per-meal granularity, caterer capability matrix, outbound email audit, dietary tag structure (see §"What this drag-in costs you" — pending).
...
8. **Schema impact (sketch — formalise later):**
   ...
   - New entity: caterer_school_capabilities (required for RFP targeting — which caterers could serve which school on which day).
...
*Next group: caterer-school capability matrix and cost comparison (Group 4 on roadmap) — natural follow-on because the rotation chain above depends on it.*
...
*Next group: Group 5 — order costing and menu history. `unit_price_at_order` (your prior: keep) vs. full effective-date menu history (your prior: defer), plus order economics snapshots (4C — scenario analysis at composition).*
...
**Canonical meal ordering produces soft cycle alignment.** A global canonical order is defined arbitrarily across the caterer's menu — say `[A, B, C, D, E, F]`. Each kid's rotation traverses the subset of that order matching their approved meals, in canonical sequence. A kid with preferences `{C, B, A}` cycles `A → B → C → A → …`; a kid with `{C, E, A}` cycles `A → C → E → A → …`. The result: kids with overlapping preferences tend to land on the same meal in any given week, which softly clusters the order without explicit clustering logic. No term-start computation, no clustering algorithm, no V4 deferral — the canonical order is a constant and each kid's cycle is deterministic given their preferences and where they started.

**MOQ handling: pay the difference, notify, continue.** If the natural composition falls below the caterer's MOQ at its natural variety, the order goes through as-is and the system pays for the MOQ floor anyway. A notification escalation fires — operator sees that this week's order at [school] underran the caterer's MOQ by N meals, costing $X. The system takes no autonomous action: doesn't pad the order, doesn't reduce variety to find a lower tier, doesn't suggest rotation. The escalation is informational. Recurring shortfalls accumulate as a pattern the operator reviews in the weekly report; sustained shortfalls feed the human rotation conversation alongside quality decline (V3-FB-01).

**Predicted cost on every order email.** The system includes its computed expected total in the order email sent to the caterer ("Expected total: $480 — 4 chicken × $12, 8 pasta × $10, 6 salad × $14, delivery $20"). This is transparency and a soft check: if the caterer's pricing has changed and they reply to flag it, the discrepancy lands as an inbound escalation. If the caterer invoices a different amount weeks later, the original predicted figure on the order row gives the operator a reference point during invoice review. The system does not ingest caterer invoices in V3, so invoice-vs-prediction variance is not an automated escalation — that's a V4 build (Shape B in the design discussion). V3 catches pricing surprises via the caterer-reply channel only.
...
5. **Complexity score:** 1/5. Two new escalation types (MOQ shortfall, caterer-declared price change). Predicted-cost line in the order email template. Weekly cost view as a query on existing data. No new tables, no new columns beyond what V3-FB-01 already adds.
6. **Bugs / fixability / onboarding risks:** Menu prices being overwrite-only means a botched manual update can't be rolled back. Mitigated by the order row carrying the predicted total at the time it was sent — even if the menu state is later wrong, the original prediction is preserved. Predicted-cost-on-email depends on the menu being accurate; a stale menu produces a wrong prediction, which the caterer's reply corrects. Canonical meal ordering being arbitrary is an operator-defined constant per caterer; if the canonical order changes mid-term, kids' rotations shift unpredictably. Recommended: set canonical order at caterer onboarding (or at preview-week end for a swap) and never change it during a caterer's tenure.
7. **Decision:** **ADD BACK, SIMPLIFIED.** Cost-per-student trajectory committed as derivation. Predicted-cost on order email committed. MOQ-shortfall and price-change escalations committed. Menu effective-date history CUT entirely. `unit_price_at_order` column CUT entirely. Order economics scenario snapshots CUT entirely. Invoice ingestion + variance escalation DEFERRED to V4. Canonical meal ordering committed as composition logic.
8. **Schema impact (sketch):**
   - No new tables.
   ...
   - Possibly one constant per caterer for canonical meal order — could live as a `canonical_menu_order` JSON column on `caterers`, or simply as a computed deterministic ordering by `menu_items.id` if we want zero stored state. Tentative call: deterministic by id — no stored constant needed, the order is whatever the id sequence produces. Worth a memo flag if the agent ever needs to expose the canonical order to the operator visually.
9. **Agent logic impact:**
   - Composition: for each kid, check `meal_requests` first; if absent, query the kid's `term_meal_preferences`, intersect with the caterer's menu in canonical order, advance to the next item past their last-served meal. Auto-pick dietary-compatible meal for restricted kids.
   ...
   - MOQ check: if `total_items < lowest_applicable_moq_tier`, log a shortfall escalation (notification, not action-required); set `total_cost_aud` to the MOQ floor cost; proceed to send.
...
10. **Notes for V4 optimisation:**
    ...
    - Stored canonical meal order with operator-editable sequence — currently deterministic-by-id.
...
- **Canonical meal ordering as soft cluster alignment.** Each kid's rotation traverses a fixed sequence; kids with overlapping preferences tend to land on the same meal in the same week without any explicit clustering. Solves the MOQ-relief problem that cycle-start alignment was going to solve, with zero clustering logic.
...
*Next group: Group 6 — preference & variety. Most of this was absorbed into V3-FB-01 already (weekly student selection, variety tracking, mid-term enrolment). Quick pass to confirm what's locked vs. what genuinely remains.*
...
**Tutor app for opt-out kids: minimal surface.** Where other kids show a 1–5 rating box, comment field, and per-week meal request picker after dinner, opt-out kids show only a small "opt-out" label in the same slot. No rating, no comment, no request picker. Tutors trained that if a kid wants meals and isn't getting them, the parent must email Padea — the tutor doesn't capture that signal in the system. This is a deliberate simplification (see memo flags) over an earlier design that had a comment box on opt-out kids.

**Mid-term enrolment: full operator handling.** The brief explicitly flags mid-term enrolment as rare. V3 doesn't build for it. When a mid-term enrolment happens, the operator handles it end-to-end: captures dietary tags and approved meal set directly (typically off a phone call or email exchange with the parent), composes the kid's first-week meal manually as part of that week's order, and enters the kid into the system with full preference data already in place. From the following week onward, the kid is in the rotation like any other enrolment — the system has no rare-case-handling logic because the operator did the handling before the system saw the data.

**Opt-out reversal: also operator-handled.** Parent later emails Padea saying their kid now wants catering. Operator captures dietary tags and approved meal set (if not already on file from a previous term), flips the opt-out flag to false, and the kid joins rotation from the following week. The system doesn't model this reversal flow because reversals are rare-case territory like mid-term enrolments.
...
1. **Feature/table/system:** Parent email with three-way response (submit / opt out / no response); opt-out flag on enrolment with tutor app showing minimal opt-out label; mid-term enrolment and opt-out reversal fully operator-handled; default-opted-in posture for new enrolments.
...
5. **Complexity score:** 1/5. One UI variant on the tutor app (opt-out label vs. standard box). One additional email response type (opt-out click). No new tables. Existing `opted_out_of_catering` column carries the state.
6. **Bugs / fixability / onboarding risks:** Tutor training must cover the "kid wants meals but is opted out → parent must email" rule, or tutors will try to override the system on the kid's behalf. The opt-out email click needs to feel like a peer of the submission button, not a hidden exit — wording matters. Operator workload for mid-term enrolments is real but bounded (brief calls them rare). Reversal flow being operator-handled means a slow-to-respond operator delays the kid's first meal by a week; acceptable for the demo, memo flag for production.
7. **Decision:** **ADD BACK, SIMPLIFIED.** Opt-out as a click in the parent email committed. Tutor-app opt-out label (no comment box) committed. Mid-term enrolment fully operator-handled — no automated handling built in V3. Opt-out reversal fully operator-handled. Default-opted-in inherited from V2.
8. **Schema impact (sketch):**
   - No new tables.
...
9. **Agent logic impact:**
   - Order composition: skip enrolments where `opted_out_of_catering = true`. No meal ordered, no slot in `order_lines`, no `meal_assignments` entry.
...
- **Removing the tutor opt-out comment box.** An earlier design had tutors recording free-text comments on opt-out kids ("kid wants meals," "kid is jealous of friends"). We cut this because the operational signal it carries (kid wants in) already has a working path (kid → parent → Padea email). Building a second channel for a signal the world already produces was infrastructure for the sake of completeness, not for the sake of value. Strong Learning-criterion sentence: "we identified that a proposed surface was a redundant channel for an existing path and chose not to build it."
...
Each outbound email lives as a row in `outbound_emails`. The row stores: intended recipient, cc list, subject, full rendered body, when it was composed, when it was sent (null until sent), status (drafted / queued_for_approval / approved / sending / sent / failed), failure reason if relevant, the agent run and step that produced it, and an email type enum (weekly_order, warning, rfp, cancellation, rfp_loser_courtesy, parent_enrolment, parent_reminder, operator_notification, other).

Status transitions are explicit. Routine, pre-approved messages (weekly orders to current incumbent) go `drafted → sending → sent`. Commercial-change messages (warning, RFP, cancellation) go `drafted → queued_for_approval → approved → sending → sent`. Failed sends transition to `failed` with the failure reason captured; the failed row triggers an escalation so the operator knows something didn't land.
...
*Urgent.* The step is gating something. Until the operator acts, an email isn't sent, an order isn't composed, a caterer isn't engaged. Examples: warning email awaiting approval, RFP awaiting approval, cancellation email awaiting approval, unclassified inbound requiring routing, Gmail send failure requiring a retry decision. These are what the operator works first on a Thursday morning.
...
1. **Feature/table/system:** Three-tier urgency classification (urgent / notable / informational) applied consistently across every escalation type. Decision log sorts and colour-codes by tier. Top-of-log summary count. No lifecycle state, no dedup, no retry, no tree, no cross-entity references column.
...
5. **Complexity score:** 1/5. One enum field on `agent_steps` (or replacing the existing free-text severity field with a constrained enum). Sort + colour logic in the decision log renderer. Summary count at the top of the log. No new tables, no new tools.
...
1. **Feature/table/system:** `tutors` table as first-class entity; `session_tutor_assignments` with `(session_id, tutor_id, is_manager, is_tutor)` for dual-role support; students and parents denormalised onto `enrolments` rows; no cross-school identity table; feedback polymorphism handles dual-role submission attribution.
...
5. **Complexity score:** 2/5. One new tutors table. One new session_tutor_assignments table. Dual-role booleans rather than role enum (cleaner queries, naturally handles all four states). No upgrades to enrolments needed; the existing denormalised student_name and parent_email columns suffice.
6. **Bugs / fixability / onboarding risks:** Tutor identity drift if the same person is entered as two rows by mistake (e.g., once with full name, once with nickname). Mitigated by operator-discipline at onboarding. Dual-role attribution requires the agent to know which kids the dual-role person tutored — comes from `session_tutor_assignments` rows where `is_tutor=true` AND a kid lookup against the assignment's session and the tutor's per-kid relationship (which V3 doesn't yet model explicitly). Memo flag: per-kid-to-tutor assignment within a session is operator-knowledge, not yet a system fact; usually the manager knows. V4 trigger documented if this becomes ambiguous.
7. **Decision:** **ADD BACK (TUTORS), CUT (STUDENTS, PARENTS, CROSS-SCHOOL).** Tutors as first-class committed (covered by V3-FB-01 schema requirement). Dual-role assignment model committed. Students denormalised on enrolment committed. Parents denormalised on enrolment committed. Cross-school identity CUT, V4 trigger documented.
8. **Schema impact (sketch):**
   ...
   - `session_slots.manager_name` and `session_slots.manager_phone` can be deprecated (manager identity now lives in assignments) or kept as fallback if no manager is assigned. Tentative: deprecate, since the assignment table is the source of truth from V3 onwards. Memo flag.
9. **Agent logic impact:**
...
**Decision summary.** V3 commits to **zero operational margin** on every order. The order is the exact count of expected attendees with their preference or request applied. No flat margin, no walkup buffer, no tutor buffer, no ML prediction adjustment. Walkups, unexpected attendees, and opted-out kids who change their mind on the day are not served; they are told to have their parents email Padea to update their status, and the system handles them from the following week. The opted-out kid's tutor app surface gains a small "request opt-back-in" checkbox that, when ticked, triggers an autonomous email to the parent asking them to confirm and submit dietary tags + approved meal set. Aside from this checkbox, every prediction, reconciliation, and buffer concept from V1 is cut.
...
**Amendment from Group 10 — opt-back-in checkbox.** The opted-out kid's tutor-app display gains one small interactive element: a checkbox labelled "student wishes to opt back in for meals." When ticked and submitted, the system autonomously sends an email to the parent (same structure as the original enrolment email) requesting confirmation, dietary tags, and approved meal set for the current caterer. No operator approval required; send-once tracking prevents repeated ticks from spamming the parent. From the next session run after the parent responds, the kid is in rotation. The tutor's recurring ticks (if the parent doesn't respond) surface as a soft signal in the weekly report. See V3-MP-01 for details.
...
**Amendment from V3-AI-01 — per-session order cadence.** Orders are now per-session, not per-week-per-caterer. Each order email at T-72hrs lists meal-name + allocated kid; feedback attaches to those per-meal allocations. Rolling per-caterer means still compute weekly. Decline detection still uses 4-week rolling vs. 12-week prior with absolute floor — the change in trigger cadence doesn't affect the math, just the rate at which fresh data arrives. Rotation chain (warning → RFP → cancellation) can fire after any session's feedback is processed, not just on Thursdays.
**Amendment from V3-AI-01 — predicted cost on both surfaces.** Predicted cost now appears in two places: per-session predicted cost on each order email (kid count × per-meal price + delivery fee proportional share), and weekly aggregated predicted cost on the Monday 3:30 PM consolidated summary (full-week totals with MOQ floor and GST applied). MOQ assessment happens against the full week's total in the summary, not against individual sessions. If a caterer's full-week total falls below MOQ, the summary email shows the floor with the variance, and the operator pays accordingly per V3-AI-01's payment-crystallisation flow.
**Amendment from V3-AI-01 — per-session cadence email types.** Two new `email_type` enum values on `outbound_emails`: `session_order` (per-session, T-72hrs, binding) and `weekly_consolidated_summary` (Monday 3:30 PM, per caterer, aggregated). The existing `weekly_order` enum value is deprecated for new emails (V2 carried it). Gmail polling now happens at the start of every agent invocation — ~5–7 polls per week instead of one — but the polling logic itself is unchanged. Idempotent classification via `gmail_message_id` prevents the same inbound email being processed twice across closely-spaced runs.
**Amendment from V3-AI-01 — decision log regenerates per run.** Under the weekly-omnibus model, the log was generated once on Thursday. Under per-session cadence, the log regenerates after every agent run (5–7 times per week). The HTML log organises events by week for readability — opens to the current week's events at the top, prior weeks collapsible below. Operator can open the log at any time and see whatever the latest snapshot is. The Monday 3:30 PM run regenerates the log with the week's complete activity grouped clearly.
**Amendment from V3-AI-01 — opt-back-in email at next agent run.** Under the weekly Thursday cadence, a tutor's opt-back-in tick on Monday evening would wait until Thursday for the email to fire. Under per-session cadence, the email fires at the next scheduled agent run — typically within 24–48 hours of the tick. If the parent responds before the next session's T-72hrs order composition, the kid is included in that session's order. Faster cycle: ~3–4 days from kid's interest to first meal, down from ~7 days under the omnibus cadence.
